...
def summarize_repository_event(event_type: str, payload: dict) -> str:
    ...
    if event_type in ['push', 'fork']:
        summary = _handle_repository_event(event_type, payload)
    elif event_type.startswith('pull_request'):
        summary = _handle_pull_request_event(event_type, payload)
...
def _handle_repository_event(event_type: str, payload: dict) -> str:
    if event_type == 'push':
...
def _handle_pull_request_event(event_type: str, payload: dict) -> str:
    return _summarize_pull_request_event(payload)
