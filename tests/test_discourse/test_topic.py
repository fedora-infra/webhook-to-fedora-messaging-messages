...
def test_topic_destroyed() -> None:
    ...
    assert msg.app_name == "Discourse"
    assert msg.summary == (
        "Topic Destroyed on Ask Fedora: pnietzsche's topic " "'Name Resolution Failure'"
    )
    assert msg.__str__() == msg.summary
    assert msg.agent_name == "pnietzsche"


def test_topic_recovered() -> None:
