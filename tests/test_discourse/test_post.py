...
def test_post_recovered() -> None:
    ...
    assert msg.app_name == "Discourse"
    assert msg.summary == (
        "Post Recovered on Fedora Discussion: hankuoffroad's post on " "'QuickDocs - Manage images'"
    )
    assert msg.__str__() == msg.summary
    assert msg.agent_name == "hankuoffroad"


def test_post_destroyed() -> None:
    ...
    assert msg.app_name == "Discourse"
    assert msg.summary == (
        "Post Destroyed on Ask Fedora: pnietzsche's post on " "'Name Resolution Failure'"
    )
    assert msg.__str__() == msg.summary
